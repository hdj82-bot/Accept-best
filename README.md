# Accept.best — 논문집필 도우미

> 한국 대학교 교수·박사과정 연구자를 위한 AI 기반 논문 작성 보조 서비스.
> 키워드 한 번으로 arXiv·Semantic Scholar에서 관련 논문을 모아 요약하고, **선행연구 기반 설문 문항**까지 자동 생성합니다.

🔗 **데모**: https://accept.best
📍 **백엔드**: https://academi-ai.onrender.com (Render, Singapore region)
📅 **현재 단계**: Sprint 7 — 베타 배포 (2026-04 기준)

---

## 제품 철학 — 답을 주는 AI가 아니라 함께 묻는 AI

Accept.best는 **단정적 결론을 제시하지 않습니다**. 모든 응답은 연구자에게 **되묻는 질문**으로 마무리되며, 연구자의 답변이 다음 단계의 입력이 됩니다.

- "이 주제로 쓰세요" (X) → "이 주제를 선정하신 이유는 무엇인가요? 세부 목차는 어떻게 구상하고 계세요?" (O)
- "이 방법론이 적합합니다" (X) → "정량/정성 중 어떤 자료가 더 충분히 모이실 것 같나요?" (O)

연구자가 깊이 사고하도록 자극하고, 누적된 익명 질문-응답 패턴이 시스템을 개선합니다. 자세한 정책은 [academi.md "대화 정책"](academi.md#대화-정책--소크라테스식-질문-기반-phase-1-핵심) 참고.
