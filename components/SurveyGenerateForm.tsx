...
import { generateSurvey, ApiError } from "@/lib/api";

export default function SurveyGenerateForm({
  onGenerated,
}: {
  onGenerated?: () => void;
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await generateSurvey(trimmed, token);
      ...
      if (err instanceof ApiError) {
        ...
        setError("설문문항 생성 중 오류가 발생했습니다. 다시 시도해 주세요.");
      }
    } finally {
      setLoading(false);
    }
